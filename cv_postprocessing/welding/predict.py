...
#     return mask
def fill_small_holes(mask, max_hole_size=100000):
    """Fill holes smaller than max_hole_size inside the mask."""
    # Invert the mask to find holes
    inverted = cv2.bitwise_not(mask)

    # Find contours in the inverted mask (these are holes)
...
def keep_largest_component(mask):
    ...
    return largest_mask
def get_mask(model, img, class_name=None, class_index=None, keep_largest=False):
    """
    Get mask for specific class by name or index

    Args:
        model: YOLO model
        img: Input image
        class_name: Class name as string (e.g., 'weld', 'plate')
        class_index: Class index as integer (e.g., 5 for weld, 2 for plate)
        keep_largest: If True, keep only the largest connected component

    Returns:
        mask: Binary mask for the specified class
        num_instances: Number of instances found (before filtering)
    """
    H, W, _ = img.shape
    results = model(img, imgsz=[640], iou=0.4, conf=0.1)
    # Map class names to indices (update with your actual class mapping)
    class_name_to_index = {
        'blue_scale': 0,
        'box scale': 1,
        'plate': 2,
    ...
    if class_name is not None:
        if class_name not in class_name_to_index:
            raise ValueError(f"Class name '{class_name}' not found. Available: {list(class_name_to_index.keys())}")
        target_index = class_name_to_index[class_name]
    elif class_index is not None:
        target_index = class_index
    else:
        raise ValueError("Either class_name or class_index must be provided")

    # Initialize empty mask and list of instance masks
    ...
    if results[0].masks is not None:
        ...
        # Collect all instances of the target class
        for i, class_id in enumerate(class_ids):
            if class_id == target_index:
                instance_mask = mask_data[i] * 255
                instance_mask = instance_mask.astype(np.uint8)
                instance_mask = cv2.resize(instance_mask, (W, H))

                instance_masks.append(instance_mask)
                num_instances += 1

        # Process instances based on keep_largest flag
        if keep_largest and instance_masks:
            ...
            for instance_mask in instance_masks:
                mask = cv2.bitwise_or(mask, instance_mask)

    return mask, num_instances
...
def process_single_image(key, img, image_path, model, ocr, output_masked, output_rendered, render, le=None, u=None, line=None):
    try:
        ...
        mask1, weld_instances = get_mask(model, im, class_name='weld', keep_largest=True)  # weld mask
        mask2, plate_instances = get_mask(model, im, class_name='plate', keep_largest=True)  # plate mask

        print(f"Found {weld_instances} weld instance(s) and {plate_instances} plate instance(s)")
        print(f"Keeping largest plate instance (out of {plate_instances})")
        ...
        mask2 = cv2.erode(mask2, kernel, iterations=10)
        ...
        mask2 = fill_small_holes(mask2, max_hole_size=100000)  # Adjust size as needed

        # Additional: Remove very small components that might still be connected
        mask2 = remove_small_components(mask2, min_size=5)
        ...
        cv2.imwrite(str(output_masked / f'{key}.jpg'), render_mask(img, mask2))
...
def process_all_images(config, ocr):
    ...
    model_path = pathlib.Path(config['model_path']).resolve()
...
def main(config_path=None, config_dict=None, ocr=None):
    ...
    if ocr is None:
        try:
            ...
            print("Warning: PaddleOCR not available. Scale detection disabled.")
            ocr = None
